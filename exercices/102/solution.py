velib = \
    [{'address': 'RUE DES CHAMPEAUX (PRES DE LA\
    GARE ROUTIERE) - 93170 BAGNOLET',
        'zip': '93170-',
        'number': 31705, 'latitude': 48.8645278209514, 'city': 'BAGNOLET',
        'name': 'CHAMPEAUX (BAGNOLET)-',
        'longitude': 2.416170724425901},
     {'address': "52 RUE D'ENGHIEN / ANGLE RUE\
    DU FAUBOURG POISSONIERE - 75010 PARIS",
        'zip': '75010',
     ...
     {'address': '139 AVENUE JEAN LOLIVE / MAIL CHARLES\
     DE GAULLE - 93500 PANTIN-',
        'zip': '93500-',
        'number': 35014,
...
def check_my_city(city_name):
    ...
    for i in (velib):
        city = i['city']
        if city[:len(city)-1] == (city_name) or city == (city_name):
            number = number + 1
            izip = i['zip']
#            if len(izip) > 5:
#                izip = izip[:5]
#                zipcode = zipcode + [izip]
#            else:
            zipcode = zipcode + [izip[:5]]
        else:
            faults = faults + 1
            if faults == len(velib):
                print("Sorry! No station for your city has been found!")
                return
    answer['stations_nb'] = number
    answer['zip_code'] = zipcode
    answer['city'] = city_name
#    answer = dict(stations_nb = number, zip_code = zipcode, city = city_name)
    print(answer)
